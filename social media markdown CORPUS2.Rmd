---
title: 'Project 5. Owen Smith''s Twitter account: using social media updates as a source of text data'
...
library(twitteR)
```
In a separate process not described here, we registered with Twitter get the Twitter keys we need to query the Twitter API.
...
```
And then use those credentials for authentication:
```{r, eval=FALSE, message=FALSE, warning=FALSE, include=TRUE}
setup_twitter_oauth(consumer_key, consumer_secret, access_token, access_secret)
```
We create a variable to store the tweets that we are going to scrape, using the userTimeline function of the `twitteR` package (Gentry, 2015:22).
```{r, eval=FALSE, message=FALSE, warning=FALSE, include=TRUE}
...
Our main interest in this data frame is the "Text" column that contains the tweets, but the rest of the parameters can provide contextual information, as we will see later on.

We start by importing `tm`, a library for text mining:
```{r, message=FALSE, warning=FALSE}
library(tm)
```
...
```
### Text data cleaning
Once we have this kind of object, we can proceed to prepare our text in a way that makes it suitable for the operations we can to perform with it later. 
...
```
Another important step in text data cleaning is the removal of stop words, that is very simple using the following method:
```{r}
tweetsCorpus <- tm_map(tweetsCorpus, removeWords, stopwords("english"))
```
### Finding frequently used terms

Now that we have cleaned the text we are in better possition to find which ones are used more frequently. To do that in R, it is necessary to create a matrix first.
...
```
We need to recreate our matrix to update it's contents (that is, exclude the "amp" results). We have only gone through the hassle of creating a new variable simply to show the process here, in practice it would simply easier to fix the original formulation and keep using the same variable (instead of tweetsMatrix2):
```{r}
...
There's a library in R that makes it very easy to generate one. We found that manipulating the `scale =` parameter is imortant in order to include all the words that we want. In this case, we had to reduce the second argument to 0.05:
```{r, message=FALSE, warning=FALSE}
library(wordcloud)
...
For example, now that we see that "support" is a commonly used word, we would try to get a better idea of in what context it is used. For that we use the following function:
```{r}
...
```
These results are a good example of how these correlations could lead to questions. It's interesting how "nikibhenry" (most likely a Twitter handle/user) has a higher correlation rate with "Labour" in something Smith tweeted.

That could be a good reason to look closer into that user's interaction with Smith.
...
First we limit the results to only those who satisfy the condition we are interested in. This returns the index of the row that contains the relevant tweet:
```{r}
...
```
We can also look at the frequency of the words sorted from the higher to the lower value, by simply displaying an interval of as many words as we want to look at (rows) and two columns: word and frequency, all that contained in the data frame `d` that we created before:
...
```
These three results show that, at least in terms of original tweets, Smith was not very prolific in social media, but this is just an opinion given the lack of a baseline for comparisons. A possible option would be to compare the number of tweets about Brexit in Jeremy Corbyn's account, as the leader of (hiw own) the party leading the Remain campaign.
```{r}
#interestingTweetsIds <- c(8, 112, 118, 119, 121)
smith_tweets_df$text[interestingTweets]

smith_tweets_df[8, 1]
smith_tweets_df[112, 1]
smith_tweets_df[118, 1]
smith_tweets_df[119, 1]
smith_tweets_df[121, 1]
```
Another element to check could be the only term in this top 20 that is a user name: mattforde.
```{r}
...
```
We check Twitter to find that this is the account of comedian [Matt Forde](https://twitter.com/mattforde). The long exchange was simple a conversatio about sports.

#### Expressions of gratitude

The associations method we saw before could give us a quick idea of where is that gratitude directed to:
```{r}
...
### Type of device(s) used by Smith
...
Let's start finding out what kind of device(s) Smith uses. We filter the column containing that information (statusSource) using the unique function, that will return unique occurances for the sources from which the tweets were posted:
```{r}
  ...
  ifelse(grepl("iPhone",smith_tweets_df$statusSource), "iPhone",
      ifelse(grepl("iPad", smith_tweets_df$statusSource), "iPad",
        ifelse(grepl("Client", smith_tweets_df$statusSource), "Web Client", 
            "Other")))
```
Note that all we would have to do to cover other kinds of devices would be to add other ifelse statements for each one. 
...
- Smith writes his own tweets using mostly a personal iPhone

- In the period studied, he only mentions his rival Jeremy Corbyn once, in a negative tweet aimed at portraying Corbyn as a threat to the Labour Party.
